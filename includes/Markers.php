...
class Marker extends DbObject
{
	...
	/** [[Column=lat, DataType=float, Description=Latitude]]*/
	public $lat;

	/** [[Column=lng, DataType=float, Description=Longitude]]*/
	public $lng;
...
*/
	...
	public function getMarkers($args=array())
	{
		...
		$query = "SELECT * 
			FROM ".self::$tableName." 
			WHERE 1=1 
				".(is_array(@$args['ids']) ? " AND id IN (".implode(',',$args['ids']).") " : "")."
			...
			ORDER BY location, floor, room_number, room_name";
